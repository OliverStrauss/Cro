...
void main() {
  testWidgets('shows loading indicator before data loads', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(MaterialApp(
      home: ProfileScreen(authState: authState, profileService: fakeProfile, friendsService: fakeFriends),
    ));
  ...
  testWidgets('shows an error and retry button on failure', (WidgetTester tester) async {
    final fakeProfile = _FakeProfileService()..loadErrorToThrow = ProfileException('Could not load profile');
    final fakeFriends = _FakeFriendsService();
    final authState = AuthState()..login(_fakeJwtFor('u1'));
    await tester.pumpWidget(MaterialApp(
      home: ProfileScreen(authState: authState, profileService: fakeProfile, friendsService: fakeFriends),
  ...
  testWidgets('renders the friends row once loaded', (WidgetTester tester) async {
    final fakeProfile = _FakeProfileService()
      ..profileToReturn = UserProfile(id: 'me', username: 'me', email: 'me@example.com');
    final fakeFriends = _FakeFriendsService()
  ...
  testWidgets('shows initials on a friend card with no profile picture', (WidgetTester tester) async {
    final fakeProfile = _FakeProfileService()
      ..profileToReturn = UserProfile(id: 'me', username: 'me', email: 'me@example.com');
    final fakeFriends = _FakeFriendsService()
      ..friendsToReturn = [Friend(userId: 'u1', username: 'alice', color: '#E53935')];
  ...
  testWidgets('invites section only shows when there are incoming requests', (WidgetTester tester) async {
    final fakeProfile = _FakeProfileService()
      ..profileToReturn = UserProfile(id: 'me', username: 'me', email: 'me@example.com');
    final authState = AuthState()..login(_fakeJwtFor('me'));
