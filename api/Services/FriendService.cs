...
public class FriendService(CosmosUserRepository userRepository, EventService eventService)
{
    public async Task SendRequestAsync(string requesterId, string targetUsername)
    {
        ...
        var target = await userRepository.GetByUsernameAsync(targetUsername)
            ?? throw new ServiceException(404, "No user with that username.");

        if (target.Id == requester.Id)
        {
            throw new ServiceException(400, "Cannot send a friend request to yourself.");
        }

        var requesterFriends = requester.Friends ?? [];
        if (requesterFriends.Any(f => f.Id == target.Id))
        {
            throw new ServiceException(409, "A relationship with this user already exists.");
        }

        if ((requester.BlockedUserIds ?? []).Contains(target.Id) || (target.BlockedUserIds ?? []).Contains(requester.Id))
        {
            throw new ServiceException(403, "Cannot send a request to this user.");
        }
    ...
    public async Task BlockAsync(string userId, string targetId)
    {
        ...
        if (userId == targetId)
        {
            throw new ServiceException(400, "Cannot block yourself.");
        }
    ...
    public async Task SetColorAsync(string userId, string friendId, string color)
    {
        ...
        {
            throw new ServiceException(400, "Not a valid palette color.");
        }
